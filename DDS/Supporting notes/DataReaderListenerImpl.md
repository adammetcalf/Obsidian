...
The header file defines the class and the class functions. This is likely to be the same for every specific implementation:
...
Note that this implementation is a concrete of the abstract class `OpenDDS::DCPS::LocalObject<DDS::DataReaderListener>`

Methods are declared, in this case the only method that we actually implement later on in the cpp file is `on_data_available`.


The corresponding c++ file is shown here:
```
...
Note that the only method which is actually implemented is `on_data_available`