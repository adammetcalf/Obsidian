...
public class TestPublisher {
    ...
    public static void main(String[] args) {
        ...
        HiTypeSupportImpl servant = new HiTypeSupportImpl();
        if (servant.register_type(dp, "") != RETCODE_OK.value) {
            System.err.println("ERROR: register_type failed");
            return;
        }

        Topic top = dp.create_topic("Hello Topic",
        ...
        HiDataWriter hdw = HiDataWriterHelper.narrow(dw);
        Hi msg = new Hi();
        msg.hiWorld = "Hello, OpenDDS!";
        int handle = hdw.register_instance(msg);
		
        int ret = RETCODE_TIMEOUT.value;
        for (int count = 0; count < N_MSGS; ++count) {
            msg.hiWorld = "Hello, OpenDDS! - " + count;
            while ((ret = hdw.write(msg, handle)) == RETCODE_TIMEOUT.value) {
            }
            if (ret != RETCODE_OK.value) {
                System.err.println("ERROR " + count + " write() returned " + ret);
            }
            try {
                Thread.sleep(100);
        ...
        while (matched.value.current_count != 0) {
            final int result = hdw.get_publication_matched_status(matched);
            try {
                Thread.sleep(100);
            } catch(InterruptedException ie) {
            }
